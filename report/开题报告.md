...
当前代码实现中，FSDP 训练入口位于训练脚本中，支持通过 FSDP 通信钩子（comm hook）在 Reduce-Scatter 前后插入梯度量化/反量化逻辑，用于实现 baseline 与压缩方案的对照。评测指标建议至少包含：后向梯度通信字节数及压缩比、每步训练耗时（step time）与训练吞吐（tokens/s）、通信算子耗时占比及通信–计算重叠率（profiler 统计）、GPU 显存峰值，以及验证集的 loss/perplexity（PPL）；必要时可辅以梯度误差统计等指标进一步分析压缩对数值稳定性的影响。
